...
#ifndef SOULNG_LEXERGEN_MODEL_INCLUDED
#define SOULNG_LEXERGEN_MODEL_INCLUDED
#include <soulng/lexergen/Nfa.hpp>
...
public:
    virtual ~Parser();
    virtual Nfa Parse(const std::u32string& expression, LexerContext* lexerContext) = 0;
};
...
{
public:
    Expression(int index_, const std::u32string& id_, const std::u32string& value_);
...
public:
    ...
    soulng::codedom::TypeId* GetType() const { return type.get(); }
    const std::u32string& Name() const { return name; }
private:
...
{
public:
    LexerStatement(const std::u32string& expr_, soulng::codedom::CompoundStatement* stmt_, int action_);
    void Process(LexerContext& lexerContext);
    void SetIndex(int index_) { index = index_; }
    int Index() const { return index; }
    const Nfa& GetNfa() const { return nfa; }
    soulng::codedom::CompoundStatement* Code() const { return stmt.get(); }
...
private:
    ...
    std::u32string expr;
    Nfa nfa;
    std::unique_ptr<soulng::codedom::CompoundStatement> stmt;
    bool retract;
    int action;
};
...
{
public:
    Action(int id_, soulng::codedom::CompoundStatement* stmt_);
...
#endif // SOULNG_LEXERGEN_MODEL_INCLUDED
