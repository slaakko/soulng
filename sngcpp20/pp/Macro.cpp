...
std::vector<Token>::const_iterator GetMacroArguments(FunctionMacro* macro, std::vector<Token>::const_iterator i, std::vector<Token>::const_iterator e, std::vector<MacroArgument>& arguments, PP* pp)
{
            ...
            case TextTokens::RPAREN:
            {
                ...
                else
                {
                    std::string error = "error: invalid invocation of function macro '" + ToUtf8(macro->Name().ToString()) + "': unexpected right parenthesis in '" +
                        pp->FileName() + ":" + std::to_string(pp->LineNumber()) + "'";
                    pp->AddError(error);
                    return e;
                }
                break;
...
std::vector<Token>::const_iterator Macro::Expand(std::vector<Token>::const_iterator i, std::vector<Token>::const_iterator e, std::vector<Token>& expandedTokens, PP* pp)
{
    return i;
}
...
std::vector<Token>::const_iterator ObjectMacro::Expand(std::vector<Token>::const_iterator i, std::vector<Token>::const_iterator e, std::vector<Token>& expandedTokens, PP* pp)
{
...
std::vector<Token>::const_iterator FunctionMacro::Expand(std::vector<Token>::const_iterator i, std::vector<Token>::const_iterator e, std::vector<Token>& expandedTokens, PP* pp)
{
    if (i != e)
    {
        ++i;
    ...
    if (i != e)
    {
        ...
        i = GetMacroArguments(this, i, e, args, pp);
        if (IsVariableArgs())
        {
            if (args.size() < paramList.size())
            {
                std::string error = "invalid invocation of variable argument function macro '" + ToUtf8(Name().ToString()) + "' in '" + pp->FileName() + ":" + std::to_string(pp->LineNumber()) +
        ...
        else
        {
            if (args.size() != paramList.size())
            {
                std::string error = "invalid invocation of function macro '" + ToUtf8(Name().ToString()) + "' in '" + pp->FileName() + ":" + std::to_string(pp->LineNumber()) +
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if (index >= 0 && index < args.size())
                                {
                                    ...
                                    if (arg.tokens.empty())
                                    {
                                        arg.tokens.push_back(Token(TextTokens::PLACEMARKER, Lexeme(), pp->LineNumber()));
                                    }
                                    for (const Token& t : arg.tokens)
                                    {
                                        expandedTokens.push_back(t);
    ...
    {
        std::string error = "invalid invocation of function macro '" + ToUtf8(Name().ToString()) + "' in '" + pp->FileName() + ":" + std::to_string(pp->LineNumber()) + 
            "': left parenthesis expected: note: definition in '" + FileName() + ":" + std::to_string(LineNumber()) + "'";
        pp->AddError(error);
        return e;
    }
    return i;
