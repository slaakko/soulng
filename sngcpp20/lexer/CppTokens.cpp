
// this file has been automatically generated from 'C:/work/soulng/sngcpp20/lexer/CppLexer.lexer' using soulng lexer generator slg version 3.10.0

#include <sngcpp20/lexer/CppTokens.hpp>
#include <map>

namespace CppTokens
{
    std::map<std::u32string, int> tokenIdMap;

    void InitTokenIdMap()
    {
        tokenIdMap[U"ADDASSIGN"] = 96;
        tokenIdMap[U"ALIGNAS"] = 1;
        tokenIdMap[U"ALIGNOF"] = 2;
        tokenIdMap[U"AMP"] = 109;
        tokenIdMap[U"AMPAMP"] = 106;
        tokenIdMap[U"ANDASSIGN"] = 100;
        tokenIdMap[U"ANGLEHEADERNAME"] = 140;
        tokenIdMap[U"ARROW"] = 137;
        tokenIdMap[U"ARROWSTAR"] = 125;
        tokenIdMap[U"ASM"] = 3;
        tokenIdMap[U"ASSIGN"] = 92;
        tokenIdMap[U"AUTO"] = 4;
        tokenIdMap[U"BEGINRAWSTRINGLIT"] = 88;
        tokenIdMap[U"BOOL"] = 5;
        tokenIdMap[U"BREAK"] = 6;
        tokenIdMap[U"CASE"] = 7;
        tokenIdMap[U"CATCH"] = 8;
        tokenIdMap[U"CHAR"] = 9;
        tokenIdMap[U"CHAR16_T"] = 11;
        tokenIdMap[U"CHAR32_T"] = 12;
        tokenIdMap[U"CHAR8_T"] = 10;
        tokenIdMap[U"CHARLIT"] = 86;
        tokenIdMap[U"CLASS"] = 13;
        tokenIdMap[U"COLON"] = 104;
        tokenIdMap[U"COLONCOLON"] = 90;
        tokenIdMap[U"COMMA"] = 91;
        tokenIdMap[U"CONCEPT"] = 14;
        tokenIdMap[U"CONST"] = 15;
        tokenIdMap[U"CONSTEVAL"] = 16;
        tokenIdMap[U"CONSTEXPR"] = 17;
        tokenIdMap[U"CONSTINIT"] = 18;
        tokenIdMap[U"CONST_CAST"] = 19;
        tokenIdMap[U"CONTINUE"] = 20;
        tokenIdMap[U"CO_AWAIT"] = 21;
        tokenIdMap[U"CO_RETURN"] = 22;
        tokenIdMap[U"CO_YIELD"] = 23;
        tokenIdMap[U"DECLTYPE"] = 24;
        tokenIdMap[U"DEFAULT"] = 25;
        tokenIdMap[U"DELETE"] = 26;
        tokenIdMap[U"DIV"] = 122;
        tokenIdMap[U"DIVASSIGN"] = 94;
        tokenIdMap[U"DO"] = 27;
        tokenIdMap[U"DOT"] = 136;
        tokenIdMap[U"DOTSTAR"] = 124;
        tokenIdMap[U"DOUBLE"] = 28;
        tokenIdMap[U"DYNAMIC_CAST"] = 29;
        tokenIdMap[U"ELLIPSES"] = 139;
        tokenIdMap[U"ELSE"] = 30;
        tokenIdMap[U"ENUM"] = 31;
        tokenIdMap[U"EQ"] = 110;
        tokenIdMap[U"EXCLAMATION"] = 130;
        tokenIdMap[U"EXPLICIT"] = 32;
        tokenIdMap[U"EXPORT"] = 33;
        tokenIdMap[U"EXTERN"] = 34;
        tokenIdMap[U"FALSE"] = 35;
        tokenIdMap[U"FLOAT"] = 36;
        tokenIdMap[U"FLOATLIT"] = 84;
        tokenIdMap[U"FOR"] = 37;
        tokenIdMap[U"FRIEND"] = 38;
        tokenIdMap[U"GEQ"] = 113;
        tokenIdMap[U"GOTO"] = 39;
        tokenIdMap[U"ID"] = 89;
        tokenIdMap[U"IF"] = 40;
        tokenIdMap[U"INLINE"] = 41;
        tokenIdMap[U"INT"] = 42;
        tokenIdMap[U"INTLIT"] = 85;
        tokenIdMap[U"LANGLE"] = 115;
        tokenIdMap[U"LBRACE"] = 134;
        tokenIdMap[U"LBRACKET"] = 132;
        tokenIdMap[U"LEQ"] = 112;
        tokenIdMap[U"LONG"] = 43;
        tokenIdMap[U"LPAREN"] = 126;
        tokenIdMap[U"MAX"] = 142;
        tokenIdMap[U"MINUS"] = 120;
        tokenIdMap[U"MINUSMINUS"] = 129;
        tokenIdMap[U"MOD"] = 123;
        tokenIdMap[U"MULASSIGN"] = 93;
        tokenIdMap[U"MUTABLE"] = 44;
        tokenIdMap[U"NAMESPACE"] = 45;
        tokenIdMap[U"NEQ"] = 111;
        tokenIdMap[U"NEW"] = 46;
        tokenIdMap[U"NOEXCEPT"] = 47;
        tokenIdMap[U"NULLPTR"] = 48;
        tokenIdMap[U"OPERATOR"] = 49;
        tokenIdMap[U"OR"] = 107;
        tokenIdMap[U"ORASSIGN"] = 102;
        tokenIdMap[U"OROR"] = 105;
        tokenIdMap[U"PLUS"] = 119;
        tokenIdMap[U"PLUSPLUS"] = 128;
        tokenIdMap[U"PRIVATE"] = 50;
        tokenIdMap[U"PROTECTED"] = 51;
        tokenIdMap[U"PUBLIC"] = 52;
        tokenIdMap[U"QUEST"] = 103;
        tokenIdMap[U"QUOTEHEADERNAME"] = 141;
        tokenIdMap[U"RANGLE"] = 116;
        tokenIdMap[U"RBRACE"] = 135;
        tokenIdMap[U"RBRACKET"] = 133;
        tokenIdMap[U"REGISTER"] = 53;
        tokenIdMap[U"REINTERPRET_CAST"] = 54;
        tokenIdMap[U"REMASSIGN"] = 95;
        tokenIdMap[U"REQUIRES"] = 55;
        tokenIdMap[U"RETURN"] = 56;
        tokenIdMap[U"RPAREN"] = 127;
        tokenIdMap[U"SEMICOLON"] = 138;
        tokenIdMap[U"SHIFTLEFT"] = 117;
        tokenIdMap[U"SHIFTLEFTASSIGN"] = 99;
        tokenIdMap[U"SHIFTRIGHT"] = 118;
        tokenIdMap[U"SHIFTRIGHTASSIGN"] = 98;
        tokenIdMap[U"SHORT"] = 57;
        tokenIdMap[U"SIGNED"] = 58;
        tokenIdMap[U"SIZEOF"] = 59;
        tokenIdMap[U"SPACESHIP"] = 114;
        tokenIdMap[U"STAR"] = 121;
        tokenIdMap[U"STATIC"] = 60;
        tokenIdMap[U"STATIC_ASSERT"] = 61;
        tokenIdMap[U"STATIC_CAST"] = 62;
        tokenIdMap[U"STRINGLIT"] = 87;
        tokenIdMap[U"STRUCT"] = 63;
        tokenIdMap[U"SUBASSIGN"] = 97;
        tokenIdMap[U"SWITCH"] = 64;
        tokenIdMap[U"TEMPLATE"] = 65;
        tokenIdMap[U"THIS"] = 66;
        tokenIdMap[U"THREAD_LOCAL"] = 67;
        tokenIdMap[U"THROW"] = 68;
        tokenIdMap[U"TILDE"] = 131;
        tokenIdMap[U"TRUE"] = 69;
        tokenIdMap[U"TRY"] = 70;
        tokenIdMap[U"TYPEDEF"] = 71;
        tokenIdMap[U"TYPEID"] = 72;
        tokenIdMap[U"TYPENAME"] = 73;
        tokenIdMap[U"UNION"] = 74;
        tokenIdMap[U"UNSIGNED"] = 75;
        tokenIdMap[U"USING"] = 76;
        tokenIdMap[U"VIRTUAL"] = 77;
        tokenIdMap[U"VOID"] = 78;
        tokenIdMap[U"VOLATILE"] = 79;
        tokenIdMap[U"WCHAR_T"] = 80;
        tokenIdMap[U"WHILE"] = 81;
        tokenIdMap[U"XOR"] = 108;
        tokenIdMap[U"XORASSIGN"] = 101;
        tokenIdMap[U"__DECLSPEC"] = 82;
        tokenIdMap[U"__THREAD"] = 83;
    }

    int GetTokenId(const std::u32string& tokenName)
    {
        auto it = tokenIdMap.find(tokenName);
        if (it != tokenIdMap.cend())
        {
            return it->second;
        }
        else
        {
            return -1;
        }
    }

    const char* tokenName[] =
    {
        "END",
        "ALIGNAS",
        "ALIGNOF",
        "ASM",
        "AUTO",
        "BOOL",
        "BREAK",
        "CASE",
        "CATCH",
        "CHAR",
        "CHAR8_T",
        "CHAR16_T",
        "CHAR32_T",
        "CLASS",
        "CONCEPT",
        "CONST",
        "CONSTEVAL",
        "CONSTEXPR",
        "CONSTINIT",
        "CONST_CAST",
        "CONTINUE",
        "CO_AWAIT",
        "CO_RETURN",
        "CO_YIELD",
        "DECLTYPE",
        "DEFAULT",
        "DELETE",
        "DO",
        "DOUBLE",
        "DYNAMIC_CAST",
        "ELSE",
        "ENUM",
        "EXPLICIT",
        "EXPORT",
        "EXTERN",
        "FALSE",
        "FLOAT",
        "FOR",
        "FRIEND",
        "GOTO",
        "IF",
        "INLINE",
        "INT",
        "LONG",
        "MUTABLE",
        "NAMESPACE",
        "NEW",
        "NOEXCEPT",
        "NULLPTR",
        "OPERATOR",
        "PRIVATE",
        "PROTECTED",
        "PUBLIC",
        "REGISTER",
        "REINTERPRET_CAST",
        "REQUIRES",
        "RETURN",
        "SHORT",
        "SIGNED",
        "SIZEOF",
        "STATIC",
        "STATIC_ASSERT",
        "STATIC_CAST",
        "STRUCT",
        "SWITCH",
        "TEMPLATE",
        "THIS",
        "THREAD_LOCAL",
        "THROW",
        "TRUE",
        "TRY",
        "TYPEDEF",
        "TYPEID",
        "TYPENAME",
        "UNION",
        "UNSIGNED",
        "USING",
        "VIRTUAL",
        "VOID",
        "VOLATILE",
        "WCHAR_T",
        "WHILE",
        "__DECLSPEC",
        "__THREAD",
        "FLOATLIT",
        "INTLIT",
        "CHARLIT",
        "STRINGLIT",
        "BEGINRAWSTRINGLIT",
        "ID",
        "COLONCOLON",
        "COMMA",
        "ASSIGN",
        "MULASSIGN",
        "DIVASSIGN",
        "REMASSIGN",
        "ADDASSIGN",
        "SUBASSIGN",
        "SHIFTRIGHTASSIGN",
        "SHIFTLEFTASSIGN",
        "ANDASSIGN",
        "XORASSIGN",
        "ORASSIGN",
        "QUEST",
        "COLON",
        "OROR",
        "AMPAMP",
        "OR",
        "XOR",
        "AMP",
        "EQ",
        "NEQ",
        "LEQ",
        "GEQ",
        "SPACESHIP",
        "LANGLE",
        "RANGLE",
        "SHIFTLEFT",
        "SHIFTRIGHT",
        "PLUS",
        "MINUS",
        "STAR",
        "DIV",
        "MOD",
        "DOTSTAR",
        "ARROWSTAR",
        "LPAREN",
        "RPAREN",
        "PLUSPLUS",
        "MINUSMINUS",
        "EXCLAMATION",
        "TILDE",
        "LBRACKET",
        "RBRACKET",
        "LBRACE",
        "RBRACE",
        "DOT",
        "ARROW",
        "SEMICOLON",
        "ELLIPSES",
        "ANGLEHEADERNAME",
        "QUOTEHEADERNAME",
        "MAX"
    };

    const char* GetTokenName(int tokenId)
    {
        return tokenName[tokenId];
    }

    const char* tokenInfo[] =
    {
        "end of file",
        "'alignas'",
        "'alignof'",
        "'asm'",
        "'auto'",
        "'bool'",
        "'break'",
        "'case'",
        "'catch'",
        "'char'",
        "'char8_t'",
        "'char16_t'",
        "'char32_t'",
        "'class'",
        "'concept'",
        "'const'",
        "'consteval'",
        "'constexpr'",
        "'constinit'",
        "'const_cast'",
        "'continue'",
        "'co_await'",
        "'co_return'",
        "'co_yield'",
        "'decltype'",
        "'default'",
        "'delete'",
        "'do'",
        "'double'",
        "'dynamic_cast'",
        "'else'",
        "'enum'",
        "'explicit'",
        "'export'",
        "'extern'",
        "'false'",
        "'float'",
        "'for'",
        "'friend'",
        "'goto'",
        "'if'",
        "'inline'",
        "'int'",
        "'long'",
        "'mutable'",
        "'namespace'",
        "'new'",
        "'noexcept'",
        "'nullptr'",
        "'operator'",
        "'private'",
        "'protected'",
        "'public'",
        "'register'",
        "'reinterpret_cast'",
        "'requires'",
        "'return'",
        "'short'",
        "'signed'",
        "'sizeof'",
        "'static'",
        "'static_assert'",
        "'static_cast'",
        "'struct'",
        "'switch'",
        "'template'",
        "'this'",
        "'thread_local'",
        "'throw'",
        "'true'",
        "'try'",
        "'typedef'",
        "'typeid'",
        "'typename'",
        "'union'",
        "'unsigned'",
        "'using'",
        "'virtual'",
        "'void'",
        "'volatile'",
        "'wchar_t'",
        "'while'",
        "'__declspec'",
        "'__thread'",
        "floating literal",
        "integer literal",
        "character literal",
        "string literal",
        "'begin raw string literal'",
        "identifier",
        "'::'",
        "','",
        "'='",
        "'*='",
        "'/='",
        "'%='",
        "'+='",
        "'-='",
        "'>>='",
        "'<<='",
        "'&='",
        "'^='",
        "'|='",
        "'?'",
        "':'",
        "'||'",
        "'&&'",
        "'|'",
        "'^'",
        "'&'",
        "'=='",
        "'!='",
        "'<='",
        "'>='",
        "<=>",
        "'<'",
        "'>'",
        "'<<'",
        "'>>'",
        "'+'",
        "'-'",
        "'*'",
        "'/'",
        "'%'",
        "'.*'",
        "'->*'",
        "'('",
        "')'",
        "'++'",
        "'--'",
        "'!'",
        "'~'",
        "'['",
        "']'",
        "'{'",
        "'}'",
        "'.'",
        "'->'",
        "';'",
        "'...'",
        "'angle header name'",
        "'quote header name'",
        "max"
    };

    const char* GetTokenInfo(int tokenId)
    {
        return tokenInfo[tokenId];
    }

    struct Initializer
    {
        Initializer() { InitTokenIdMap(); }
    };

    Initializer initializer;
}
