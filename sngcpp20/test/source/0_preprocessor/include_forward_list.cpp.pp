...
public:
    ...
    _Flist_unchecked_const_iterator operator++(int) noexcept {
     ...
     bool operator==(const _Flist_unchecked_const_iterator& _Right) const noexcept {
...
public:
    ...
    _Flist_const_iterator operator++(int) noexcept {
     ...
     bool operator==(const _Flist_const_iterator& _Right) const noexcept {
    ...
    using _Prevent_inheriting_unwrap = _Flist_const_iterator;
     ...
     _Flist_unchecked_const_iterator<_Mylist> _Unwrapped() const noexcept {
...
public:
    ...
    using _Prevent_inheriting_unwrap = _Flist_iterator;
     ...
     _Flist_unchecked_iterator<_Mylist> _Unwrapped() const noexcept {
...
struct _Flist_node {      using _Nodeptr = _Rebind_pointer_t<_Voidptr, _Flist_node>;

    _Nodeptr _Next;      _Value_type _Myval;  
    ...
    static void _Freenode(_Alnode& _Al, _Nodeptr _Pnode) noexcept {
        ...
        allocator_traits<_Alnode>::destroy(_Al, ::std:: addressof(_Pnode->_Myval));
...
public:
    ...
    _Flist_val() noexcept : _Myhead() {}  
    _Nodeptr _Before_head() const noexcept {          return pointer_traits<_Nodeptr>::pointer_to(reinterpret_cast<_Node&>(const_cast<_Nodeptr&>(_Myhead)));
    }
    ...
    static _Nodeptr _Inplace_merge(_Nodeptr _BFirst1, const _Nodeptr _BMid, const _Nodeptr _BLast, _Pr2 _Pred) {
        ...
        for (;;) {              _Nodeptr _First1;
            for (;;) {                  if (_BFirst1 == _BMid) {
                    return _BLast;
                }

                _First1 = _BFirst1->_Next;
                if (static_cast<bool>(_Pred(_First2->_Myval, _First1->_Myval))) {
                                         break;
                }
            ...
            for (;;) {
                ...
                if (!static_cast<bool>(_Pred(_Run_end->_Myval, _First1->_Myval))) {
                                                                                   break;
                }
            ...
            _BMid->_Next     = _Run_end;              _BFirst1->_Next  = _First2;              _BRun_end->_Next = _First1;
            if (_BRun_end == _BLast) {
                return _BMid;
            }
    ...
    static _Nodeptr _Sort2(const _Nodeptr _BFirst, _Pr2 _Pred) {
        ...
        if (!_First2 || !static_cast<bool>(_Pred(_First2->_Myval, _First1->_Myval))) {
            return _First1;
        }

                 _First1->_Next = _First2->_Next;          _BFirst->_Next = _First2;          _First2->_Next = _First1;
        return _First2;
    }
...
struct _Flist_insert_after_op2 {
    ...
    void _Append_n(typename _Alnode_traits::size_type _Count, const _CArgT&... _Carg) {
        ...
        if (_Tail == pointer{}) {
            _Newnode._Allocate();              _Alnode_traits::construct(_Al, ::std:: addressof(_Newnode._Ptr->_Myval), _Carg...);              _Head = _Newnode._Ptr;
            _Tail = _Newnode._Ptr;
            --_Count;
        }

        for (; 0 < _Count; --_Count) {
            _Newnode._Allocate();              _Alnode_traits::construct(_Al, ::std:: addressof(_Newnode._Ptr->_Myval), _Carg...);              _Construct_in_place(_Tail->_Next, _Newnode._Ptr);
            _Tail = _Newnode._Ptr;
        }
    ...
    void _Append_range_unchecked(_InIt _First, const _Sentinel _Last) {
        ...
        _Alloc_construct_ptr<_Alnode> _Newnode(_Al);
        if (_Tail == pointer{}) {
            _Newnode._Allocate();              _Alnode_traits::construct(_Al, ::std:: addressof(_Newnode._Ptr->_Myval), *_First);              const auto _Newhead = _Newnode._Release();
            _Head               = _Newhead;
            _Tail               = _Newhead;
            ++_First;          }

        while (_First != _Last) {              _Newnode._Allocate();              _Alnode_traits::construct(_Al, ::std:: addressof(_Newnode._Ptr->_Myval), *_First);              const auto _Newtail = _Newnode._Release();
...
private:
    _Alnode& _Al;
    pointer _Tail;                          pointer _Head;  };

 template <class _Ty, class _Alloc = allocator<_Ty>>
class forward_list {  private:
...
public:
    ...
    forward_list() noexcept(is_nothrow_default_constructible_v<_Alnode>)          : _Mypair(_Zero_then_variadic_args_t{}) {
        _Alloc_proxy();
    }

    explicit forward_list( size_type _Count, const _Alloc& _Al = _Alloc())
        : _Mypair(_One_then_variadic_args_t{}, _Al) {          _Flist_insert_after_op2<_Alnode> _Insert_op(_Getal());
    ...
    forward_list( size_type _Count, const _Ty& _Val)
        : _Mypair(_Zero_then_variadic_args_t{}) {          _Flist_insert_after_op2<_Alnode> _Insert_op(_Getal());
    ...
    explicit forward_list(const _Alloc& _Al) noexcept          : _Mypair(_One_then_variadic_args_t{}, _Al) {
        _Alloc_proxy();
    }
    ...
    forward_list(forward_list&& _Right) noexcept          : _Mypair(_One_then_variadic_args_t{}, ::std:: move(_Right._Getal())) {
        _Alloc_proxy();
        _Take_head(_Right);
    }

    forward_list(forward_list&& _Right, const _Alloc& _Al) noexcept(
        _Alnode_traits::is_always_equal::value)          : _Mypair(_One_then_variadic_args_t{}, _Al) {
        if constexpr (!_Alty_traits::is_always_equal::value) {
            if (_Getal() != _Right._Getal()) {
                _Flist_insert_after_op2<_Alnode> _Insert_op(_Getal());
                _Insert_op._Append_range_unchecked(
                    ::std:: make_move_iterator(_Right._Unchecked_begin()), _Default_sentinel{});
                _Alloc_proxy();
                _Insert_op._Attach_after(_Mypair._Myval2._Before_head());
                return;
            }
...
private:
    ...
    void _Move_assign(forward_list& _Right, _No_propagate_allocators) {
        if (_Getal() == _Right._Getal()) {
            ...
            _Assign_unchecked(::std:: make_move_iterator(_Right._Unchecked_begin()), _Right._Unchecked_end());
        }
    }
...
private:
    void _Take_head(forward_list& _Right) noexcept {          _Swap_proxy_and_iterators(_Right);
        _Mypair._Myval2._Myhead = ::std:: exchange(_Right._Mypair._Myval2._Myhead, nullptr);
    }

public:
    void push_front(_Ty&& _Val) {          _Insert_after(_Mypair._Myval2._Before_head(), ::std:: move(_Val));
    }

    iterator insert_after(const_iterator _Where, _Ty&& _Val) {          return emplace_after(_Where, ::std:: move(_Val));
    }

    template <class... _Valty>
    decltype(auto) emplace_front(_Valty&&... _Val) {          _Insert_after(_Mypair._Myval2._Before_head(), ::std:: forward<_Valty>(_Val)...);

        return front();
    }

    template <class... _Valty>
    iterator emplace_after(const_iterator _Where, _Valty&&... _Val) {          _Insert_after(_Where._Ptr, ::std:: forward<_Valty>(_Val)...);
        return _Make_iter(_Where._Ptr->_Next);
    }

private:
    template <class... _Valty>
    void _Insert_after(_Nodeptr _Pnode, _Valty&&... _Val) {          _Alloc_construct_ptr<_Alnode> _Newnode(_Getal());
        _Newnode._Allocate();          _Alnode_traits::construct(
            _Newnode._Al, ::std:: addressof(_Newnode._Ptr->_Myval), ::std:: forward<_Valty>(_Val)...);          _Construct_in_place(_Newnode._Ptr->_Next, _Pnode->_Next);
...
public:
    ...
    iterator insert_after(const_iterator _Where,
        initializer_list<_Ty> _Ilist) {          return insert_after(_Where, _Ilist.begin(), _Ilist.end());
    }
...
public:
    forward_list& operator=(const forward_list& _Right) {
     ...
     iterator before_begin() noexcept {
        return iterator(_Mypair._Myval2._Before_head(), ::std:: addressof(_Mypair._Myval2));
    }

     const_iterator before_begin() const noexcept {
        return const_iterator(_Mypair._Myval2._Before_head(), ::std:: addressof(_Mypair._Myval2));
    }
     ...
     iterator end() noexcept {
        return iterator(nullptr, ::std:: addressof(_Mypair._Myval2));
    }

     const_iterator end() const noexcept {
        return const_iterator(nullptr, ::std:: addressof(_Mypair._Myval2));
    }
...
private:
    template <class... _Args>
    void _Resize( size_type _Newsize, const _Args&... _Vals) {
        auto& _Al = _Getal();
...
private:
    ...
    void _Assign_unchecked(_UIter _UFirst, _Sentinel _ULast) {
        ...
        for (auto _To_delete = ::std:: exchange(_Myfirst->_Next, nullptr); _To_delete;) {
...
public:
    ...
    iterator insert_after(const_iterator _Where, const _Ty& _Val) {          _Insert_after(_Where._Ptr, _Val);
        return _Make_iter(_Where._Ptr->_Next);
    }
...
private:
    void _Erase_after(_Nodeptr _Pnode) noexcept {          auto _Subject = _Pnode->_Next;
        _Pnode->_Next = _Subject->_Next;
        _Node::_Freenode(_Getal(), _Subject);
    }

public:
    iterator erase_after(const_iterator _Where) noexcept   {
        _Erase_after(_Where._Ptr);
        return _Make_iter(_Where._Ptr->_Next);
    }

    iterator erase_after(const_iterator _First, const_iterator _Last) noexcept   {
    ...
    void clear() noexcept {  
        _Nodeptr _Pnext;
        _Nodeptr _Pnode = ::std:: exchange(_Mypair._Myval2._Myhead, nullptr);

        auto& _Al = _Getal();
        for (; _Pnode; _Pnode = _Pnext) {              _Pnext = _Pnode->_Next;
    ...
    void splice_after(const_iterator _Where, forward_list& _Right) noexcept   {
         
        if (this != ::std:: addressof(_Right) && !_Right.empty()) {              _Splice_after(_Where._Unwrapped(), _Right, _Right._Unchecked_before_begin(), _Default_sentinel{});
        }
    }
    ...
    void splice_after(const_iterator _Where, forward_list& _Right, const_iterator _First) noexcept   {
    ...
    struct _Flist_node_remove_op {
        ...
        explicit _Flist_node_remove_op(forward_list& _List_) noexcept
            : _List(_List_), _Head(), _Tail(::std:: addressof(_Head)) {}
    ...
    auto remove(const _Ty& _Val) {          return remove_if([&](const _Ty& _Other) { return _Other == _Val; });
    }

    template <class _Pr1>
    auto remove_if(_Pr1 _Pred) {          _Flist_node_remove_op _Op(*this);
    ...
    auto unique() {          return unique(equal_to<>{});
    }

    template <class _Pr2>
    auto unique(_Pr2 _Pred) {          _Flist_node_remove_op _Op(*this);
        auto _First        = _Unchecked_begin();
        size_type _Removed = 0;
        if (_First._Ptr) {              auto _After = _First;
            ++_After;
            while (_After._Ptr) {
    ...
    void merge(forward_list& _Right) {          _Merge1(_Right, less<>{});
    }
    ...
    template <class _Pr2>
    void merge(forward_list& _Right, _Pr2 _Pred) {          _Merge1(_Right, _Pass_fn(_Pred));
    }
...
private:
    template <class _Pr2>
    void _Merge1(forward_list& _Right, _Pr2 _Pred) {          if (this == ::std:: addressof(_Right)) {
            return;
        }
        ...
        for (;;) {              _Nodeptr _First1;
            for (;;) {                  _First1 = _BFirst1->_Next;
                if (!_First1) {                      _BFirst1->_Next     = _First2;
                    _Right_data._Myhead = nullptr;
                    return;
                }
            ...
            _Right_data._Myhead = _Run_end;              _BFirst1->_Next     = _First2;              _BRun_end->_Next    = _First1;
            if (!_Run_end) {
                return;
            }
...
public:
    void sort() {          _Scary_val::_Sort(_Mypair._Myval2._Before_head(), less<>{});
    }

    template <class _Pr2>
    void sort(_Pr2 _Pred) {          _Scary_val::_Sort(_Mypair._Myval2._Before_head(), _Pass_fn(_Pred));
    }

    void reverse() noexcept {          auto _Current = _Mypair._Myval2._Myhead;
        if (!_Current) {
                         return;
        }
...
template <class _Ty, class _Alloc>
void swap(forward_list<_Ty, _Alloc>& _Left, forward_list<_Ty, _Alloc>& _Right) noexcept   {
    _Left.swap(_Right);
}
...
namespace pmr {
    template <class _Ty>
    using forward_list = ::std:: forward_list<_Ty, polymorphic_allocator<_Ty>>;
}  }
