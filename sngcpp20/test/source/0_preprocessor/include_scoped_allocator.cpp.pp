...
public:
    ...
    template <class _Ty, class... _Types>
    void construct(_Ty* _Ptr, _Types&&... _Args) {          ::std:: apply(
            [_Ptr, this](auto&&... _New_args) {
                _Scoped_outermost_traits<scoped_allocator_adaptor>::construct(
                    _Scoped_outermost(*this), _Ptr, ::std:: forward<decltype(_New_args)>(_New_args)...);
            },
            ::std:: uses_allocator_construction_args<_Ty>(inner_allocator(), ::std:: forward<_Types>(_Args)...));
    }
