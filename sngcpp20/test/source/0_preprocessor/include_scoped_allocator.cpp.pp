...
namespace std {
 template <class _Alloc, class = void>
struct _Scoped_outermost_helper {      static decltype(auto) _Fn(_Alloc& _Al) {          return _Al;
    }
};

template <class _Alloc>
struct _Scoped_outermost_helper<_Alloc,
    void_t<decltype(::std:: declval<_Alloc&>().outer_allocator())>> {      using _Outer_alloc = decltype(::std:: declval<_Alloc&>().outer_allocator());

    static decltype(auto) _Fn(_Alloc& _Al) {          return _Scoped_outermost_helper<_Outer_alloc>::_Fn(_Al.outer_allocator());
    }
};

template <class _Alloc>
decltype(auto) _Scoped_outermost(_Alloc& _Al) {      return _Scoped_outermost_helper<_Alloc>::_Fn(_Al);
}

 template <class _Alloc>
using _Scoped_outermost_t = remove_reference_t<decltype(_Scoped_outermost(::std:: declval<_Alloc&>()))>;
...
struct _Scoped_base<_Outer, _Inner0, _Inner...> : _Outer {      using _Myadaptor           = scoped_allocator_adaptor<_Outer, _Inner0, _Inner...>;
    ...
    _Scoped_base() : _Outer(), _Inner_obj() {}  
    template <class _Other1, class... _Other2,
    ...
    _Scoped_base(_Other1&& _Outer_arg, _Other2&&... _Inner_args)
        : _Outer(::std:: forward<_Other1>(_Outer_arg)), _Inner_obj(::std:: forward<_Other2>(_Inner_args)...) {
             }
    ...
    _Scoped_base& operator=(_Scoped_base&&) = default;
     ...
     _Myadaptor select_on_container_copy_construction() const {          return _Myadaptor(_Secret_scoped_allocator_construct_tag{},
...
struct _Scoped_base<_Outer> : _Outer {      using _Myadaptor           = scoped_allocator_adaptor<_Outer>;
    ...
    _Scoped_base(_Other1&& _Outer_arg) : _Outer(::std:: forward<_Other1>(_Outer_arg)) {}  
    _Scoped_base(const _Scoped_base&) = default;
    _Scoped_base(_Scoped_base&&)      = default;
    _Scoped_base& operator=(const _Scoped_base&) = default;
    _Scoped_base& operator=(_Scoped_base&&) = default;

     _Myadaptor select_on_container_copy_construction() const {          return _Myadaptor(
            allocator_traits<_Outer>::select_on_container_copy_construction(static_cast<const _Outer&>(*this)));
    }
};

 template <class _Outer, class... _Inner>
class scoped_allocator_adaptor : public _Scoped_base<_Outer, _Inner...> {      using _Mybase       = _Scoped_base<_Outer, _Inner...>;
...
public:
    ...
    template <class _Other>
    struct rebind {          using _Other_alloc = typename _Get_rebind_type<_Outer, _Other>::type;
    ...
    scoped_allocator_adaptor(_Other&& _Other_arg, const _Inner&... _Inner_args) noexcept
        : _Mybase(::std:: forward<_Other>(_Other_arg), _Inner_args...) {
             }

    scoped_allocator_adaptor(const scoped_allocator_adaptor& _Right) noexcept : _Mybase(_Right) {}

    scoped_allocator_adaptor(scoped_allocator_adaptor&& _Right) noexcept : _Mybase(::std:: move(_Right)) {}
    ...
    scoped_allocator_adaptor(scoped_allocator_adaptor<_Other, _Inner...>&& _Right) noexcept
        : _Mybase(::std:: move(_Right.outer_allocator()), ::std:: move(_Right.inner_allocator())) {}
    ...
    scoped_allocator_adaptor& operator=(scoped_allocator_adaptor&&) = default;
     ...
     inner_allocator_type& inner_allocator() noexcept {          return this->_Get_inner_object(*this);
    }

     const inner_allocator_type& inner_allocator() const noexcept {          return this->_Get_inner_object(*this);
    }

     outer_allocator_type& outer_allocator() noexcept {          return static_cast<_Outer&>(*this);
    }

     const outer_allocator_type& outer_allocator() const noexcept {          return static_cast<const _Outer&>(*this);
    }

     __declspec(allocator) pointer
        allocate( size_type _Count) {          return _Outer_traits::allocate(outer_allocator(), _Count);
    }

     __declspec(allocator) pointer allocate(
         size_type _Count, const_void_pointer _Hint) {          return _Outer_traits::allocate(outer_allocator(), _Count, _Hint);
    }

    void deallocate(pointer _Ptr, size_type _Count) {          return _Outer_traits::deallocate(outer_allocator(), _Ptr, _Count);
    }

     size_type max_size() const {          return _Outer_traits::max_size(outer_allocator());
    }

    template <class _Ty, class... _Types>
    void construct(_Ty* _Ptr, _Types&&... _Args) {          _Uses_allocator_construct(_Ptr, _Scoped_outermost(*this), inner_allocator(), ::std:: forward<_Types>(_Args)...);
    }

    template <class _Ty>
    void destroy(_Ty* _Ptr) {          _Scoped_outermost_traits<scoped_allocator_adaptor>::destroy(_Scoped_outermost(*this), _Ptr);
    }
...
template <class _Outer1, class _Outer2, class _Inner1, class... _Inner>
 bool operator==(const scoped_allocator_adaptor<_Outer1, _Inner1, _Inner...>& _Left,
    const scoped_allocator_adaptor<_Outer2, _Inner1, _Inner...>&
        _Right) noexcept {      return _Left.outer_allocator() == _Right.outer_allocator() && _Left.inner_allocator() == _Right.inner_allocator();
}

template <class _Outer1, class _Outer2>
 bool operator==(const scoped_allocator_adaptor<_Outer1>& _Left,
    const scoped_allocator_adaptor<_Outer2>& _Right) noexcept {      return _Left.outer_allocator() == _Right.outer_allocator();
}
...
template <class _Outer1, class _Outer2, class... _Inner>
 bool operator!=(const scoped_allocator_adaptor<_Outer1, _Inner...>& _Left,
    const scoped_allocator_adaptor<_Outer2, _Inner...>&
        _Right) noexcept {      return !(_Left == _Right);
}
