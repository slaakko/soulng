...
struct _Any_big_RTTI {      using _Destroy_fn = void __cdecl(void*) noexcept;
    using _Copy_fn    = void* __cdecl(const void*);

    template <class _Ty>
    static void __cdecl _Destroy_impl(void* const _Target) noexcept {
        ::delete static_cast<_Ty*>(_Target);
    }

    template <class _Ty>
     static void* __cdecl _Copy_impl(const void* const _Source) {
...
struct _Any_small_RTTI {      using _Destroy_fn = void __cdecl(void*) noexcept;
    using _Copy_fn    = void __cdecl(void*, const void*);
    using _Move_fn    = void __cdecl(void*, void*) noexcept;
    ...
    template <class _Ty>
    static void __cdecl _Copy_impl(void* const _Target, const void* const _Source) {
        _Construct_in_place(*static_cast<_Ty*>(_Target), *static_cast<const _Ty*>(_Source));
    }

    template <class _Ty>
    static void __cdecl _Move_impl(void* const _Target, void* const _Source) noexcept {
        _Construct_in_place(*static_cast<_Ty*>(_Target), ::std:: move(*static_cast<_Ty*>(_Source)));
    }
...
inline constexpr _Any_small_RTTI _Any_small_RTTI_obj = {
 ...
 class any {  public:
    ...
    any(const any& _That) {
        ...
        default:
            :: memcpy(_Storage._TrivialData, _That._Storage._TrivialData, sizeof(_Storage._TrivialData));
            break;
        }
    }
    ...
    template <class _ValueType, enable_if_t<conjunction_v<negation<is_same<decay_t<_ValueType>, any>>,
                                    ...
                                    int> = 0>
    any(_ValueType&& _Value) {          _Emplace<decay_t<_ValueType>>(::std:: forward<_ValueType>(_Value));
    }
    ...
    explicit any(in_place_type_t<_ValueType>, _Types&&... _Args) {
                 _Emplace<decay_t<_ValueType>>(::std:: forward<_Types>(_Args)...);
    }
    ...
    explicit any(in_place_type_t<_ValueType>, initializer_list<_Elem> _Ilist, _Types&&... _Args) {
                 _Emplace<decay_t<_ValueType>>(_Ilist, ::std:: forward<_Types>(_Args)...);
    }
    ...
    any& operator=(_ValueType&& _Value) {
                 *this = any{::std:: forward<_ValueType>(_Value)};
        return *this;
    }
    ...
    decay_t<_ValueType>& emplace(_Types&&... _Args) {
                 reset();
        return _Emplace<decay_t<_ValueType>>(::std:: forward<_Types>(_Args)...);
    }
    template <class _ValueType, class _Elem, class... _Types,
        enable_if_t<conjunction_v<is_constructible<decay_t<_ValueType>, initializer_list<_Elem>&, _Types...>,
                        is_copy_constructible<decay_t<_ValueType>>>,
    ...
    decay_t<_ValueType>& emplace(initializer_list<_Elem> _Ilist, _Types&&... _Args) {
                 reset();
        return _Emplace<decay_t<_ValueType>>(_Ilist, ::std:: forward<_Types>(_Args)...);
    }

    void reset() noexcept {          switch (_Rep()) {
        case _Any_representation::_Small:
    ...
    void swap(any& _That) noexcept {
        _That = ::std:: exchange(*this, ::std:: move(_That));
    }
    ...
    template <class _Decayed>
     const _Decayed* _Cast() const noexcept {
                 const type_info* const _Info = _TypeInfo();
    ...
    template <class _Decayed>
     _Decayed* _Cast() noexcept {          return const_cast<_Decayed*>(static_cast<const any*>(this)->_Cast<_Decayed>());
    }

private:
    static constexpr uintptr_t _Rep_mask = 3;

     _Any_representation _Rep() const noexcept {          return static_cast<_Any_representation>(_Storage._TypeData & _Rep_mask);
    }
     const type_info* _TypeInfo() const noexcept {          return reinterpret_cast<const type_info*>(_Storage._TypeData & ~_Rep_mask);
    }
    ...
    _Decayed& _Emplace(_Types&&... _Args) {          if constexpr (_Any_is_trivial<_Decayed>) {
                         auto& _Obj = reinterpret_cast<_Decayed&>(_Storage._TrivialData);
            _Construct_in_place(_Obj, ::std:: forward<_Types>(_Args)...);
            _Storage._TypeData =
            ...
            return _Obj;
                         ...
                         _Decayed* const _Ptr       = ::new _Decayed(::std:: forward<_Types>(_Args)...);
...
template <class _ValueType, class... _Types>
 any make_any(_Types&&... _Args) {      return any{in_place_type<_ValueType>, ::std:: forward<_Types>(_Args)...};
}
template <class _ValueType, class _Elem, class... _Types>
 any make_any(initializer_list<_Elem> _Ilist, _Types&&... _Args) {
         return any{in_place_type<_ValueType>, _Ilist, ::std:: forward<_Types>(_Args)...};
}

template <class _ValueType>
 const _ValueType* any_cast(const any* const _Any) noexcept {
...
template <class _ValueType>
 _ValueType* any_cast(any* const _Any) noexcept {
         static_assert(!is_void_v<_ValueType>, "std::any cannot contain void.");
...
template <class _Ty>
 remove_cv_t<_Ty> any_cast(const any& _Any) {
    static_assert(is_constructible_v<remove_cv_t<_Ty>, const _Remove_cvref_t<_Ty>&>,
        "any_cast<T>(const any&) requires remove_cv_t<T> to be constructible from const remove_cv_t<remove_reference_t<T>>&");

    const auto _Ptr = ::std:: any_cast<_Remove_cvref_t<_Ty>>(&_Any);
    if (!_Ptr) {
        _Throw_bad_any_cast();
    }
...
template <class _Ty>
 remove_cv_t<_Ty> any_cast(any& _Any) {
