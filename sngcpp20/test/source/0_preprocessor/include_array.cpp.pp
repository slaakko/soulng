...
public:
    ...
    constexpr _Array_const_iterator() noexcept : _Ptr() {}

    constexpr explicit _Array_const_iterator(pointer _Parg, size_t _Off = 0) noexcept : _Ptr(_Parg + _Off) {}
    ...
    constexpr _Array_const_iterator& operator+=(const ptrdiff_t _Off) noexcept {
     ...
     constexpr _Array_const_iterator operator+(const ptrdiff_t _Off) const noexcept {
        _Array_const_iterator _Tmp = *this;
        return _Tmp += _Off;
    }
    ...
    constexpr _Array_const_iterator& operator-=(const ptrdiff_t _Off) noexcept {
     ...
     constexpr ptrdiff_t operator-(const _Array_const_iterator& _Right) const noexcept {
     ...
     constexpr bool operator==(const _Array_const_iterator& _Right) const noexcept {
        return _Ptr == _Right._Ptr;
    }

     constexpr strong_ordering operator<=>(const _Array_const_iterator& _Right) const noexcept {
        return _Ptr <=> _Right._Ptr;
    }
...
template <class _Ty, size_t _Size>
 constexpr _Array_const_iterator<_Ty, _Size> operator+(
...
public:
    ...
    constexpr _Array_iterator& operator+=(const ptrdiff_t _Off) noexcept {
     ...
     constexpr _Array_iterator operator+(const ptrdiff_t _Off) const noexcept {
        _Array_iterator _Tmp = *this;
        return _Tmp += _Off;
    }
...
template <class _Ty, size_t _Size>
 constexpr _Array_iterator<_Ty, _Size> operator+(
...
class array {  public:
    ...
    constexpr void swap(array& _Other) noexcept(_Is_nothrow_swappable<_Ty>::value) {
...
struct _Enforce_same {
    static_assert(conjunction_v<is_same<_First, _Rest>...>,
        "N4687 26.3.7.2 [array.cons]/2: Requires: (is_same_v<T, U> && ...) is true. Otherwise the program is ill-formed.");
    using type = _First;
};
...
constexpr void swap(array<_Ty, _Size>& _Left, array<_Ty, _Size>& _Right) noexcept(noexcept(_Left.swap(_Right))) {
    return _Left.swap(_Right);
}

template <class _Ty, size_t _Size>
 constexpr bool operator==(const array<_Ty, _Size>& _Left, const array<_Ty, _Size>& _Right) {
    return ::std:: equal(_Left._Unchecked_begin(), _Left._Unchecked_end(), _Right._Unchecked_begin());
}


template <class _Ty, size_t _Size>
 constexpr _Synth_three_way_result<_Ty> operator<=>(
 ...
 constexpr array<remove_cv_t<_Ty>, _Size> _To_array_lvalue_impl(
    _Ty (&_Array)[_Size], index_sequence<_Idx...>) {
    return {{_Array[_Idx]...}};
}

template <class _Ty, size_t _Size, size_t... _Idx>
 constexpr array<remove_cv_t<_Ty>, _Size> _To_array_rvalue_impl(
    _Ty(&&_Array)[_Size], index_sequence<_Idx...>) {
    return {{::std:: move(_Array[_Idx])...}};
}

template <class _Ty, size_t _Size>
 constexpr array<remove_cv_t<_Ty>, _Size> to_array(_Ty (&_Array)[_Size]) {
    static_assert(!is_array_v<_Ty>, "N4830 [array.creation]/1: to_array does not accept multidimensional arrays.");
    static_assert(is_constructible_v<_Ty, _Ty&>, "N4830 [array.creation]/1: to_array requires copy constructible elements.");
    return _To_array_lvalue_impl(_Array, make_index_sequence<_Size>{});
}

template <class _Ty, size_t _Size>
 constexpr array<remove_cv_t<_Ty>, _Size> to_array(_Ty(&&_Array)[_Size]) {
    static_assert(!is_array_v<_Ty>, "N4830 [array.creation]/4: to_array does not accept multidimensional arrays.");
    static_assert(is_move_constructible_v<_Ty>, "N4830 [array.creation]/4: to_array requires move constructible elements.");
    return _To_array_rvalue_impl(::std:: move(_Array), make_index_sequence<_Size>{});
}
