...
#include <soulng/lexergen/Algorithm.hpp>
...
void Tokens::Process(const std::string& root, bool verbose, LexerContext& lexerContext)
{
    ...
    sourceFileFormatter.WriteLine("static void Init()");
    sourceFileFormatter.WriteLine("{");
    ...
    sourceFileFormatter.WriteLine("}");
    sourceFileFormatter.WriteLine();
    sourceFileFormatter.WriteLine("static const char* tokenName[] =");
    sourceFileFormatter.WriteLine("{");
    sourceFileFormatter.IncIndent();
    ...
    sourceFileFormatter.WriteLine("}");
    sourceFileFormatter.WriteLine();
    sourceFileFormatter.WriteLine("static const char* tokenInfo[] =");
    sourceFileFormatter.WriteLine("{");
    sourceFileFormatter.IncIndent();
    ...
    sourceFileFormatter.WriteLine("{");
    sourceFileFormatter.IncIndent();
    sourceFileFormatter.WriteLine("Initializer() { Init(); }");
    sourceFileFormatter.DecIndent();
    sourceFileFormatter.WriteLine("};");
    sourceFileFormatter.WriteLine();
...
void Keywords::Process(const std::string& root, bool verbose, LexerContext& lexerContext)
{
    ...
    sourceFormatter.WriteLine();
    sourceFormatter.WriteLine("static Keyword keywords[] =");
    sourceFormatter.WriteLine("{");
    sourceFormatter.IncIndent();
    bool first = true;
    ...
    sourceFormatter.WriteLine("};");
    sourceFormatter.WriteLine();
    sourceFormatter.WriteLine("static KeywordMap keywordMap(keywords);");
    sourceFormatter.WriteLine();
...
Expression::Expression(int index_, const std::u32string& id_, const std::u32string& value_) : index(index_), id(id_), value(value_)
{
}
...
Variable::Variable(soulng::codedom::TypeId* type_, const std::u32string& name_) : type(type_), name(name_)
{
}

LexerStatement::LexerStatement(const std::u32string& expr_, soulng::codedom::CompoundStatement* stmt_, int action_) : index(-1), expr(expr_), stmt(stmt_), retract(true), action(action_)
{
}

void LexerStatement::Process(LexerContext& lexerContext)
{
    nfa = lexerContext.GetParser()->Parse(expr, &lexerContext);
...
void Lexer::WriteAutomaton(const std::string& root, bool verbose, LexerContext& lexerContext)
{
    ...
    for (const auto& statement : statements)
    {
        ...
        statement->Code()->Add(new soulng::codedom::BreakStatement());
...
Action::Action(int id_, soulng::codedom::CompoundStatement* stmt_) : id(id_), stmt(stmt_)
{
}
