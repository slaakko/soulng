...
#include <soulng/lexergen/Identifier.hpp>
...
LexerContext::LexerContext(bool useAsciiIdentifierClasses) :
...
Nfa LexerContext::MakeExpr(const std::u32string& id)
{
    ...
    if (expressions)
    {
        ...
        if (parser)
        {
            ...
            Nfa nfa = parser->Parse(expr->Value(), this);
