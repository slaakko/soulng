...
public:
    ...
    ActionParser(const std::u32string& name_, soulng::cppcode::CompoundStatement* successCode_, Parser* child_);
    ActionParser(const std::u32string& name_, soulng::cppcode::CompoundStatement* successCode_, soulng::cppcode::CompoundStatement* failCode_, 
        Parser* child_);
    bool IsActionParser() const override { return true; }
    std::u32string MethodName() const;
    std::u32string VariableName() const;
    soulng::cppcode::CompoundStatement* SuccessCode() const { return successCode.get(); }
    soulng::cppcode::CompoundStatement* FailCode() const { return failCode.get(); }
...
private:
    ...
    std::unique_ptr<soulng::cppcode::CompoundStatement> failCode;
