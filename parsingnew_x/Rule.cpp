...
using soulng::cppcode::IdExpr;

class CodeExpandingVisitor : public soulng::cppcode::Visitor
{
public:
    ...
    ParsingObject(name_, grammar_->GetScope(), ObjectKind::ruleLink), linkedRuleName(linkedRuleName_), grammar(grammar_)
{
    ...
    if (prefix == U"stdlib")
    {
        linkedRuleName = U"soulng.parsing.stdlib." + GetCommonName(linkedRuleName);
    }
}
