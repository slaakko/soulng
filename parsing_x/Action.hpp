...
#ifndef SOULNG_PARSING_ACTION_INCLUDED
#define SOULNG_PARSING_ACTION_INCLUDED
#include <soulng/parsing/Composite.hpp>
#include <soulng/parsing/Scanner.hpp>
...
public:
    ...
    ActionParser(const std::u32string& name_, soulng::codedom::CompoundStatement* successCode_, Parser* child_);
    ActionParser(const std::u32string& name_, soulng::codedom::CompoundStatement* successCode_, soulng::codedom::CompoundStatement* failCode_, 
        Parser* child_);
    bool IsActionParser() const override { return true; }
    std::u32string MethodName() const;
    std::u32string VariableName() const;
    soulng::codedom::CompoundStatement* SuccessCode() const { return successCode.get(); }
    soulng::codedom::CompoundStatement* FailCode() const { return failCode.get(); }
...
private:
    std::unique_ptr<soulng::codedom::CompoundStatement> successCode;
    std::unique_ptr<soulng::codedom::CompoundStatement> failCode;
...
#endif // SOULNG_PARSING_ACTION_INCLUDED
