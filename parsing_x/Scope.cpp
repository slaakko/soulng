...
#include <soulng/parsing/Utility.hpp>
...
void Scope::AddNamespace(Namespace* nsToAdd)
{
    ...
    std::vector<std::u32string> nameComponents = soulng::util::Split(nsToAdd->FullName(), '.');
...
{
    std::vector<std::u32string> components = soulng::util::Split(qualifiedObjectName, '.');
