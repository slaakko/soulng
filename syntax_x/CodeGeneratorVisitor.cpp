...
#include <soulng/syntax/CodeGeneratorVisitor.hpp>
...
using soulng::unicode::ToUtf32;
using soulng::util::StringStr;
using soulng::util::CharStr;
using soulng::util::Replace;

class KeywordListCreator : public soulng::parsing::Visitor
{
public:
...
void CodeGeneratorVisitor::BeginVisit(Grammar& grammar)
{
    ...
    hppFormatter.WriteLine("class " + ToUtf8(grammar.Name()) + " : public soulng::parsing::Grammar");
    ...
    hppFormatter.WriteLine("static " + grammarPtrType + " Create(soulng::parsing::ParsingDomain* parsingDomain);");
    ...
    cppFormatter.WriteLine("return Create(new soulng::parsing::ParsingDomain());");
    cppFormatter.DecIndent();
    cppFormatter.WriteLine("}");
    cppFormatter.NewLine();

    cppFormatter.WriteLine(grammarPtrType + " " + ToUtf8(grammar.Name()) + "::Create(soulng::parsing::ParsingDomain* parsingDomain)");
    cppFormatter.WriteLine("{");
    cppFormatter.IncIndent();
    ...
    cppFormatter.WriteLine(ToUtf8(grammar.Name()) + "::" + ToUtf8(grammar.Name()) + "(soulng::parsing::ParsingDomain* parsingDomain_): soulng::parsing::Grammar(ToUtf32(\"" + ToUtf8(grammar.Name()) + 
    ...
    if (grammar.StartRule())
    {
        if (grammar.StartRule()->TakesOrReturnsValuesOrHasLocals())
        {
            ...
            cppFormatter.WriteLine("{");
            cppFormatter.IncIndent();
            cppFormatter.WriteLine("soulng::parsing::Scanner scanner(start, end, fileName, fileIndex, SkipRule());");
            cppFormatter.WriteLine("std::unique_ptr<soulng::parsing::XmlLog> xmlLog;");
            cppFormatter.WriteLine("if (Log())");
            cppFormatter.WriteLine("{");
            cppFormatter.IncIndent();
            cppFormatter.WriteLine("xmlLog.reset(new soulng::parsing::XmlLog(*Log(), MaxLogLineLength()));");
            cppFormatter.WriteLine("scanner.SetLog(xmlLog.get());");
            cppFormatter.WriteLine("xmlLog->WriteBeginRule(\"parse\");");
            cppFormatter.DecIndent();
            cppFormatter.WriteLine("}");
            cppFormatter.WriteLine("soulng::parsing::ObjectStack stack;");
            cppFormatter.WriteLine("std::unique_ptr<soulng::parsing::ParsingData> parsingData(new soulng::parsing::ParsingData(GetParsingDomain()->GetNumRules()));");
            ...
            {
                const AttrOrVariable& attr = grammar.StartRule()->InheritedAttributes()[i];
                cppFormatter.WriteLine("stack.push(std::unique_ptr<soulng::parsing::Object>(new ValueObject<" + ToUtf8(attr.TypeName()) + ">(" + ToUtf8(attr.Name()) + ")));");
            }

            cppFormatter.WriteLine("soulng::parsing::Match match = soulng::parsing::Grammar::Parse(scanner, stack, parsingData.get());");
            cppFormatter.WriteLine("soulng::parsing::Span stop = scanner.GetSpan();");
            cppFormatter.WriteLine("if (Log())");
            cppFormatter.WriteLine("{");
            cppFormatter.IncIndent();
            ...
            cppFormatter.WriteLine("{");
            cppFormatter.IncIndent();
            cppFormatter.WriteLine("throw soulng::parsing::ExpectationFailure(StartRule()->Info(), fileName, stop, start, end);");
            cppFormatter.DecIndent();
            cppFormatter.WriteLine("}");
            cppFormatter.WriteLine("else");
            cppFormatter.WriteLine("{");
            cppFormatter.IncIndent();
            cppFormatter.WriteLine("throw soulng::parsing::ParsingException(\"grammar '\" + ToUtf8(Name()) + \"' has no start rule\", fileName, scanner.GetSpan(), start, end);");
            cppFormatter.DecIndent();
            cppFormatter.WriteLine("}");
            cppFormatter.DecIndent();
            cppFormatter.WriteLine("}");
            ...
            {
                cppFormatter.WriteLine("std::unique_ptr<soulng::parsing::Object> value = std::move(stack.top());");
                cppFormatter.WriteLine(valueTypeName + " result = *static_cast<soulng::parsing::ValueObject<" + valueTypeName + ">*>(value.get());");
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        if (rule->Specialized())
        {
            cppFormatter.WriteLine("class " + ToUtf8(grammar.Name()) + "::" + ToUtf8(rule->SpecializedTypeName()) + " : public soulng::parsing::Rule");
            cppFormatter.WriteLine("{");
            ...
            cppFormatter.IncIndent();
            cppFormatter.WriteLine("soulng::parsing::Rule(name_, enclosingScope_, id_, definition_)");
            cppFormatter.DecIndent();
            cppFormatter.WriteLine("{");
            cppFormatter.IncIndent();
            ...
            // enter:
            cppFormatter.WriteLine("void Enter(soulng::parsing::ObjectStack& stack, soulng::parsing::ParsingData* parsingData) override");
            cppFormatter.WriteLine("{");
            cppFormatter.IncIndent();
            ...
            {
                const AttrOrVariable& attr = rule->InheritedAttributes()[i];
                cppFormatter.WriteLine("std::unique_ptr<soulng::parsing::Object> " + ToUtf8(attr.Name()) + "_value = std::move(stack.top());");
                cppFormatter.WriteLine("context->" + ToUtf8(attr.Name()) + " = *static_cast<soulng::parsing::ValueObject<" + ToUtf8(attr.TypeName()) + ">*>(" + ToUtf8(attr.Name()) + "_value.get());");
                cppFormatter.WriteLine("stack.pop();");
            }
            cppFormatter.DecIndent();
            cppFormatter.WriteLine("}");
            // leave:
            cppFormatter.WriteLine("void Leave(soulng::parsing::ObjectStack& stack, soulng::parsing::ParsingData* parsingData, bool matched) override");
            cppFormatter.WriteLine("{");
            cppFormatter.IncIndent();
            if (!rule->ValueTypeName().empty())
            {
                ...
                cppFormatter.WriteLine("{");
                cppFormatter.IncIndent();
                cppFormatter.WriteLine("stack.push(std::unique_ptr<soulng::parsing::Object>(new soulng::parsing::ValueObject<" + ToUtf8(rule->ValueTypeName()) + ">(context->value)));");
                cppFormatter.DecIndent();
                cppFormatter.WriteLine("}");
            }
            ...
            for (int i = 0; i < m; ++i)
            {
                ...
                cppFormatter.WriteLine("soulng::parsing::ActionParser* " + ToUtf8(action->VariableName()) + " = GetAction(ToUtf32(\"" + ToUtf8(action->Name()) + "\"));");
                cppFormatter.WriteLine(ToUtf8(action->VariableName()) + "->SetAction(new soulng::parsing::MemberParsingAction<" + ToUtf8(rule->SpecializedTypeName()) + ">(this, &" + 
                    ToUtf8(rule->SpecializedTypeName()) + "::" + ToUtf8(action->MethodName()) + "));");
                if (action->FailCode())
                {
                    cppFormatter.WriteLine(ToUtf8(action->VariableName()) + "->SetFailureAction(new soulng::parsing::MemberFailureAction<" + ToUtf8(rule->SpecializedTypeName()) + ">(this, &" + 
            ...
            for (int i = 0; i < m; ++i)
            {
                ...
                {
                    cppFormatter.WriteLine("soulng::parsing::NonterminalParser* " + ToUtf8(nonterminal->VariableName()) + " = GetNonterminal(ToUtf32(\"" + ToUtf8(nonterminal->Name()) + "\"));");
                    if (!nonterminal->Arguments().empty())
                    {
                        cppFormatter.WriteLine(ToUtf8(nonterminal->VariableName()) + "->SetPreCall(new soulng::parsing::MemberPreCall<" + ToUtf8(rule->SpecializedTypeName()) + ">(this, &" + 
                            ToUtf8(rule->SpecializedTypeName()) + "::" + ToUtf8(nonterminal->PreCallMethodName()) + "));");
                    }
                    if (!nonterminal->GetRule()->ValueTypeName().empty())
                    {
                        cppFormatter.WriteLine(ToUtf8(nonterminal->VariableName()) + "->SetPostCall(new soulng::parsing::MemberPostCall<" + ToUtf8(rule->SpecializedTypeName()) + ">(this, &" + 
            ...
            for (int i = 0; i < m; ++i)
            {
                ...
                if (nonterminal->Specialized())
                {
                    if (!nonterminal->Arguments().empty())
                    {
                        cppFormatter.WriteLine("void " + ToUtf8(nonterminal->PreCallMethodName()) + "(soulng::parsing::ObjectStack& stack, ParsingData* parsingData)");
                        cppFormatter.WriteLine("{");
                        cppFormatter.IncIndent();
                        cppFormatter.WriteLine("Context* context = static_cast<Context*>(parsingData->GetContext(Id()));");
                        ...
                        for (int j = 0; j < p; ++j)
                        {
                            soulng::codedom::CppObject* argument = nonterminal->Arguments()[j];
                            std::string argumentTypeName = ToUtf8(nonterminal->GetRule()->InheritedAttributes()[j].TypeName());
                            cppFormatter.Write("stack.push(std::unique_ptr<soulng::parsing::Object>(new soulng::parsing::ValueObject<" + argumentTypeName + ">(");
                    ...
                    if (!nonterminal->GetRule()->ValueTypeName().empty())
                    {
                        cppFormatter.WriteLine("void " + ToUtf8(nonterminal->PostCallMethodName()) + "(soulng::parsing::ObjectStack& stack, ParsingData* parsingData, bool matched)");
                        cppFormatter.WriteLine("{");
                        cppFormatter.IncIndent();
                        cppFormatter.WriteLine("Context* context = static_cast<Context*>(parsingData->GetContext(Id()));");
                        cppFormatter.WriteLine("if (matched)");
                        cppFormatter.WriteLine("{");
                        cppFormatter.IncIndent();
                        cppFormatter.WriteLine("std::unique_ptr<soulng::parsing::Object> " + ToUtf8(nonterminal->ValueFieldName()) + "_value = std::move(stack.top());");
                        cppFormatter.WriteLine("context->" + ToUtf8(nonterminal->ValueFieldName()) + " = *static_cast<soulng::parsing::ValueObject<" + 
            ...
            // Context:
            cppFormatter.WriteLine("struct Context : soulng::parsing::Context");
            cppFormatter.WriteLine("{");
            cppFormatter.IncIndent();
    ...
    {
        cppFormatter.WriteLine("soulng::parsing::ParsingDomain* pd = GetParsingDomain();");
        ...
        for (GrammarSetIt i = grammar.GrammarReferences().cbegin(); i != e; ++i)
        {
            ...
            ++grammarIndex;
            cppFormatter.WriteLine("soulng::parsing::Grammar* " + grammarVar + " = pd->GetGrammar(ToUtf32(\"" + grammarReferenceName + "\"));");
...
void CodeGeneratorVisitor::EndVisit(Grammar& grammar)
{
    ...
    hppFormatter.WriteLine(ToUtf8(grammar.Name()) + "(soulng::parsing::ParsingDomain* parsingDomain_);");
...
{
    cppFormatter.Write("new soulng::parsing::CharParser('");
...
{
    cppFormatter.Write("new soulng::parsing::StringParser(ToUtf32(\"" + ToUtf8(StringStr(parser.GetString())) + "\"))");
}

void CodeGeneratorVisitor::Visit(CharSetParser& parser)
{
    cppFormatter.Write("new soulng::parsing::CharSetParser(ToUtf32(\"" + ToUtf8(StringStr(parser.Set())) + "\")");
    if (parser.Inverse())
    {
...
void CodeGeneratorVisitor::Visit(KeywordParser& parser)
{
    ...
    {
        cppFormatter.Write("new soulng::parsing::KeywordParser(ToUtf32(\"" + ToUtf8(StringStr(parser.Keyword())) + "\"))");
    }
    else
    {
        cppFormatter.Write("new soulng::parsing::KeywordParser(ToUtf32(\"" + ToUtf8(StringStr(parser.Keyword())) + "\"), ToUtf32(\"" + ToUtf8(parser.ContinuationRuleName()) + "\"))");
    }
}

void CodeGeneratorVisitor::Visit(KeywordListParser& parser)
{
    cppFormatter.Write("new soulng::parsing::KeywordListParser(ToUtf32(\"" + ToUtf8(StringStr(parser.SelectorRuleName())) + "\"), " + ToUtf8(parser.KeywordVecName()) + ")");
}

void CodeGeneratorVisitor::Visit(RangeParser& parser)
{
    cppFormatter.Write("new soulng::parsing::RangeParser(" + std::to_string(parser.Start()) + ", " + std::to_string(parser.End()) + ")");
}

void CodeGeneratorVisitor::Visit(EmptyParser& parser)
{
    cppFormatter.Write("new soulng::parsing::EmptyParser()");
}

void CodeGeneratorVisitor::Visit(SpaceParser& parser)
{
    cppFormatter.Write("new soulng::parsing::SpaceParser()");
}

void CodeGeneratorVisitor::Visit(LetterParser& parser)
{
    cppFormatter.Write("new soulng::parsing::LetterParser()");
}

void CodeGeneratorVisitor::Visit(UpperLetterParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::UpperLetterParser()");
}

void CodeGeneratorVisitor::Visit(LowerLetterParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::LowerLetterParser()");
}

void CodeGeneratorVisitor::Visit(TitleLetterParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::TitleLetterParser()");
}

void CodeGeneratorVisitor::Visit(ModifierLetterParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::ModifierLetterParser()");
}

void CodeGeneratorVisitor::Visit(OtherLetterParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::OtherLetterParser()");
}

void CodeGeneratorVisitor::Visit(CasedLetterParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::CasedLetterParser()");
}

void CodeGeneratorVisitor::Visit(DigitParser& parser)
{
    cppFormatter.Write("new soulng::parsing::DigitParser()");
}

void CodeGeneratorVisitor::Visit(HexDigitParser& parser)
{
    cppFormatter.Write("new soulng::parsing::HexDigitParser()");
}

void CodeGeneratorVisitor::Visit(MarkParser& parser)
{
    cppFormatter.Write("new soulng::parsing::MarkParser()");
}

void CodeGeneratorVisitor::Visit(NonspacingMarkParser& parser)
{
    cppFormatter.Write("new soulng::parsing::NonspacingMarkParser()");
}

void CodeGeneratorVisitor::Visit(SpacingMarkParser& parser)
{
    cppFormatter.Write("new soulng::parsing::SpacingMarkParser()");
}

void CodeGeneratorVisitor::Visit(EnclosingMarkParser& parser)
{
    cppFormatter.Write("new soulng::parsing::EnclosingMarkParser()");
}

void CodeGeneratorVisitor::Visit(NumberParser& parser)
{
    cppFormatter.Write("new soulng::parsing::NumberParser()");
}

void CodeGeneratorVisitor::Visit(DecimalNumberParser& parser)
{
    cppFormatter.Write("new soulng::parsing::DecimalNumberParser()");
}

void CodeGeneratorVisitor::Visit(LetterNumberParser& parser)
{
    cppFormatter.Write("new soulng::parsing::LetterNumberParser()");
}

void CodeGeneratorVisitor::Visit(OtherNumberParser& parser)
{
    cppFormatter.Write("new soulng::parsing::OtherNumberParser()");
}

void CodeGeneratorVisitor::Visit(PunctuationParser& parser)
{
    cppFormatter.Write("new soulng::parsing::PunctuationParser()");
}

void CodeGeneratorVisitor::Visit(ConnectorPunctuationParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::ConnectorPunctuationParser()");
}

void CodeGeneratorVisitor::Visit(DashPunctuationParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::DashPunctuationParser()");
}

void CodeGeneratorVisitor::Visit(OpenPunctuationParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::OpenPunctuationParser()");
}

void CodeGeneratorVisitor::Visit(ClosePunctuationParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::ClosePunctuationParser()");
}

void CodeGeneratorVisitor::Visit(InitialPunctuationParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::InitialPunctuationParser()");
}

void CodeGeneratorVisitor::Visit(FinalPunctuationParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::FinalPunctuationParser()");
}

void CodeGeneratorVisitor::Visit(OtherPunctuationParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::OtherPunctuationParser()");
}

void CodeGeneratorVisitor::Visit(SymbolParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::SymbolParser()");
}

void CodeGeneratorVisitor::Visit(MathSymbolParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::MathSymbolParser()");
}

void CodeGeneratorVisitor::Visit(CurrencySymbolParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::CurrencySymbolParser()");
}

void CodeGeneratorVisitor::Visit(ModifierSymbolParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::ModifierSymbolParser()");
}

void CodeGeneratorVisitor::Visit(OtherSymbolParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::OtherSymbolParser()");
}

void CodeGeneratorVisitor::Visit(SeparatorParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::SeparatorParser()");
}

void CodeGeneratorVisitor::Visit(SpaceSeparatorParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::SpaceSeparatorParser()");
}

void CodeGeneratorVisitor::Visit(LineSeparatorParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::LineSeparatorParser()");
}

void CodeGeneratorVisitor::Visit(ParagraphSeparatorParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::ParagraphSeparatorParser()");
}

void CodeGeneratorVisitor::Visit(OtherParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::OtherParser()");
}

void CodeGeneratorVisitor::Visit(ControlParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::ControlParser()");
}

void CodeGeneratorVisitor::Visit(FormatParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::FormatParser()");
}

void CodeGeneratorVisitor::Visit(SurrogateParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::SurrogateParser()");
}

void CodeGeneratorVisitor::Visit(PrivateUseParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::PrivateUseParser()");
}

void CodeGeneratorVisitor::Visit(UnassignedParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::UnassignedParser()");
}

void CodeGeneratorVisitor::Visit(GraphicParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::GraphicParser()");
}

void CodeGeneratorVisitor::Visit(BaseCharParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::BaseCharParser()");
}

void CodeGeneratorVisitor::Visit(AlphabeticParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::AlphabeticParser()");
}

void CodeGeneratorVisitor::Visit(IdStartParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::IdStartParser()");
}

void CodeGeneratorVisitor::Visit(IdContParser& parser) 
{
    cppFormatter.Write("new soulng::parsing::IdContParser()");
}

void CodeGeneratorVisitor::Visit(AnyCharParser& parser)
{
    cppFormatter.Write("new soulng::parsing::AnyCharParser()");
}

void CodeGeneratorVisitor::Visit(NonterminalParser& parser)
{
    cppFormatter.Write("new soulng::parsing::NonterminalParser(ToUtf32(\"" + ToUtf8(parser.Name()) + "\"), ToUtf32(\"" + ToUtf8(parser.RuleName()) + "\"), " + 
        std::to_string(parser.NumberOfArguments()) + ")");
}

void CodeGeneratorVisitor::Visit(RuleLink& link)
{
    cppFormatter.WriteLine("AddRuleLink(new soulng::parsing::RuleLink(ToUtf32(\"" + ToUtf8(link.Name()) + "\"), this, ToUtf32(\"" + ToUtf8(link.LinkedRuleName()) + "\")));");
}
...
void CodeGeneratorVisitor::BeginVisit(Rule& rule)
{
    ...
    {
        cppFormatter.Write("AddRule(new soulng::parsing::Rule(ToUtf32(\"" + ToUtf8(rule.Name()) + "\"), GetScope(), GetParsingDomain()->GetNextRuleId(),");
    }
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
...
{
    cppFormatter.Write("new soulng::parsing::OptionalParser(");
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
...
{
    cppFormatter.Write("new soulng::parsing::PositiveParser(");
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
...
{
    cppFormatter.Write("new soulng::parsing::KleeneStarParser(");
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
...
{
    cppFormatter.Write("new soulng::parsing::ActionParser(ToUtf32(\"" + ToUtf8(parser.Name()) + "\"),");
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
...
{
    cppFormatter.Write("new soulng::parsing::ExpectationParser(");
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
...
{
    cppFormatter.Write("new soulng::parsing::TokenParser(");
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
...
{
    cppFormatter.Write("new soulng::parsing::GroupingParser(");
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
...
{
    cppFormatter.Write("new soulng::parsing::SequenceParser(");
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
...
{
    cppFormatter.Write("new soulng::parsing::AlternativeParser(");
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
...
{
    cppFormatter.Write("new soulng::parsing::DifferenceParser(");
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
...
{
    cppFormatter.Write("new soulng::parsing::IntersectionParser(");
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
...
{
    cppFormatter.Write("new soulng::parsing::ExclusiveOrParser(");
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
...
{
    cppFormatter.Write("new soulng::parsing::ListParser(");
    cppFormatter.IncIndent();
    cppFormatter.NewLine();
}
