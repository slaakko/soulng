...
void Primary::GetReferencedGrammars()
{
    ...
    soulng::parsing::Grammar* grammar0 = pd->GetGrammar(ToUtf32("soulng.code.Statement"));
    if (!grammar0)
    {
        grammar0 = soulng::code::Statement::Create(pd);
    }
    AddGrammarReference(grammar0);
    soulng::parsing::Grammar* grammar1 = pd->GetGrammar(ToUtf32("soulng.syntax.Primitive"));
    if (!grammar1)
    {
        grammar1 = soulng::syntax::Primitive::Create(pd);
    }
    ...
    soulng::parsing::Grammar* grammar4 = pd->GetGrammar(ToUtf32("soulng.syntax.Element"));
    if (!grammar4)
    {
        grammar4 = soulng::syntax::Element::Create(pd);
    }
