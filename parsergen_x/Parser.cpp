...
ActionParser::ActionParser(soulng::codedom::CompoundStatement* successCode_, soulng::codedom::CompoundStatement* failCode_, Parser* child_) :
...
Parameter::Parameter(soulng::codedom::TypeId* type_, const std::u32string& name_) : type(type_), name(name_), variable(false)
{
}

Parameter::Parameter(soulng::codedom::TypeId* type_, const std::u32string& name_, bool variable_) : type(type_), name(name_), variable(variable_)
{
}
...
Variable::Variable(soulng::codedom::TypeId* type_, const std::u32string& name_) : Parameter(type_, name_, true)
{
}
...
void NonterminalParser::SetArguments(const std::vector<soulng::codedom::CppObject*>& args_)
{
    args = args_;
    for (const auto& arg : args)
    {
        ...
        {
            arg->SetOwned();
            ownedArgs.push_back(std::unique_ptr<soulng::codedom::CppObject>(arg));
        }
    }
}

RuleParser::RuleParser(const std::u32string& name_, Parser* definition_) : UnaryParser(name_, definition_), hasReturn(false)
{
}

void RuleParser::SetParameters(std::vector<Parameter*>& params)
{
    for (Parameter* param : params)
    {
        if (param->variable)
        {
            variables.push_back(std::unique_ptr<Variable>(static_cast<Variable*>(param)));
        }
        else
        {
            parameters.push_back(std::unique_ptr<Parameter>(param));
        }
    }
}

void RuleParser::SetReturnType(soulng::codedom::TypeId* returnType_)
{
