...
void InvokeExpr::AddExpr(CppObject* arg)
{
    Own(arg);
    arguments.push_back(arg);
}
...
BinaryOpExpr::BinaryOpExpr(CppObject* left_, Operator op_, CppObject* right_): BinaryExpression(GetOperatorStr(op_), left_, right_), op(op_)
{
}
...
NewExpr::NewExpr(bool global_) : Expression(U"new"), global(global_), addPlacement(false), addInitializer(false), placement(), typeId(), parens(), initializer()
{
}

void NewExpr::AddExpr(CppObject* expr)
{
    Own(expr);
    if (addPlacement)
    {
        placement.push_back(expr);
    }
    else if (addInitializer)
    {
        initializer.push_back(expr);
    }
}

void NewExpr::SetTypeId(TypeId* typeId_)
{
    Own(typeId_);
    typeId = typeId_;
}
...
ParenExpr::ParenExpr(CppObject* expr_) : Expression(U"()"), expr(expr_)
{
    Own(expr);
}

void ParenExpr::Print(CodeFormatter& formatter)
{
    formatter.Write("(");
    expr->Print(formatter);
    formatter.Write(")");
