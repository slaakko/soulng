...
#pragma warning(default: 4244)
    ...
    hppFormatter.WriteLine("#include <soulng/parsing/Grammar.hpp>");
    hppFormatter.WriteLine("#include <soulng/parsing/Keyword.hpp>");
    if (hppFileName != "StdLib.hpp")
    {
        cppFormatter.WriteLine("#include \"" + hppFileName + "\"");
    }
    cppFormatter.WriteLine("#include <soulng/parsing/Action.hpp>");
    cppFormatter.WriteLine("#include <soulng/parsing/Rule.hpp>");
    cppFormatter.WriteLine("#include <soulng/parsing/ParsingDomain.hpp>");
    cppFormatter.WriteLine("#include <soulng/parsing/Primitive.hpp>");
    cppFormatter.WriteLine("#include <soulng/parsing/Composite.hpp>");
    cppFormatter.WriteLine("#include <soulng/parsing/Nonterminal.hpp>");
    cppFormatter.WriteLine("#include <soulng/parsing/Exception.hpp>");
    cppFormatter.WriteLine("#include <soulng/parsing/StdLib.hpp>");
    cppFormatter.WriteLine("#include <soulng/parsing/XmlLog.hpp>");
    ...
    for (NamespaceContentMapIt i = nsContent.begin(); i != e; ++i)
    {
        ...
        if (!content.UsingObjects().empty())
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                soulng::codedom::UsingObject* subject = usingObject->Subject();
        ...
        cppFormatter.WriteLine("using namespace soulng::parsing;");
        cppFormatter.WriteLine("using namespace soulng::util;");
        cppFormatter.WriteLine("using namespace soulng::unicode;");
        cppFormatter.WriteLine();
